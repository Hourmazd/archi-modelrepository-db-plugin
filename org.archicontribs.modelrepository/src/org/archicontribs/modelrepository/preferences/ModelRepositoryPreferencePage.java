...
import org.archicontribs.modelrepository.db.sql.SQLServerConfiguration;
 ...
 */
public class ModelRepositoryPreferencePage extends PreferencePage
	...
	private String[] DATABASE_ENGINES = { Messages.ModelRepositoryPreferencePage_28,
	...
	private void testConnection() {
		...
		if (fDatabaseEngine.getText().equals(Messages.ModelRepositoryPreferencePage_28)) {
			...
			parameters.UserName = fUserNameTextField.getText();

			if (serverPasswordChanged) {
				parameters.Password = fUserPasswordTextField.getText();
			} else {
				EncryptedCredentialsStorage dbCredentials = getServerCredentials();
				try {
					parameters.Password = String.valueOf(dbCredentials.getPassword());
				} catch (Exception e) {
					e.printStackTrace();
	...
	public boolean performOk() {
		...
		getPreferenceStore().setValue(PREFS_FETCH_IN_BACKGROUND_INTERVAL,
				fFetchInBackgroundIntervalSpinner.getSelection());
	...
	protected void performDefaults() {
		...
		fFetchInBackgroundIntervalSpinner
				.setSelection(getPreferenceStore().getDefaultInt(PREFS_FETCH_IN_BACKGROUND_INTERVAL));
	}
